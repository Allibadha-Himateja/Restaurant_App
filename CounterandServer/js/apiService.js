...
export const apiService = {
    ...
    // -- Table Endpoints --
    getTables: () => apiRequest('/tables', 'GET'),
    ...
    addItemsToOrder: (id, items) => apiRequest(`/orders/${id}/items`, 'POST', { items }),
    ...
    serveKitchenItem: (queueId, orderItemId) => {
        return apiRequest('/kitchen/serve-item', 'PATCH', { queueId, orderItemId });
    },

    // -- Bill Endpoints --
    getBills: () => apiRequest('/bills', 'GET'),
    generateBill: (orderId) => apiRequest('/bills/generate', 'POST', { orderId }),
    updateBillStatus: (id, status) => apiRequest(`/bills/${id}/status`, 'PATCH', { status }),
};
